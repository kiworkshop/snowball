...
const NoteDetailContainer: React.FC<NoteContainerProps> = ({ id }) => {
  ...
  const { note, loading, error } = useSelector((state: RootState) => state.note);

  const getNote             = useCallback((id: number) => dispatch(getNoteAsync.request(id)), [dispatch]);
  const deleteNote          = useCallback((id: number) => dispatch(deleteNoteAsync.request(id)), [dispatch]);
  const onClickUpdateButton = useCallback((id: number) => () => history.push(routes.note.update(id)), [history]);
